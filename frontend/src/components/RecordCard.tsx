...
function feedingLabel(t: Feeding['type']): string {
	return { formula: '奶粉', breast_bottle: '母乳瓶喂', breast_left: '左乳', breast_right: '右乳', breast_both: '双侧' }[t]
}
...
export default function RecordCard({
	...
	// breast_bottle has amount like formula; other breast types have duration
	const sub = (() => {
		if (feeding) {
			if (feeding.type === 'formula' || feeding.type === 'breast_bottle') return `${feeding.amount ?? 0}ml`
