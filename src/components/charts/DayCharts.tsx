...
export default function DayCharts({ feedings, diapers }: { feedings: Feeding[]; diapers: Diaper[] }) {
  const hourlyData = useMemo(() => {
    ...
    for (const f of feedings) {
      ...
      if (f.type === 'formula') map[h].ml += f.amount ?? 0
      else map[h].min += Math.round((f.durationSec ?? 0) / 60)
    }
  ...
  const feedingPie = useMemo(() => {
    ...
    for (const f of feedings) {
      const key = f.type === 'formula' ? '奶粉' : f.type === 'breast_left' ? '左乳' : f.type === 'breast_right' ? '右乳' : '双侧'
      map[key] = (map[key] || 0) + 1
    }
          ...
          {diaperPie.length > 0 && (
            <div>
              <p className="text-fluid-xs text-ink-600 mb-1">
                便便情况{totalRash > 0 ? ` · 红臀 ${totalRash} 次` : ''}
              </p>
              <ResponsiveContainer width="100%" height={160}>
                <PieChart>
                  <Pie
          ...
          <div className="flex flex-col gap-1.5 max-h-56 overflow-y-auto pr-1">
            {[
              ...feedings.map((f) => ({
                ...
                label: f.type === 'formula'
                  ? `${f.amount ?? 0}ml 奶粉`
                  : `${Math.floor((f.durationSec ?? 0) / 60)}分钟 ${f.type === 'breast_left' ? '左乳' : f.type === 'breast_right' ? '右乳' : '双侧'}`,
